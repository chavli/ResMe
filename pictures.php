...
<html>
    ...
    <link rel='stylesheet' href='/public/css/sitemenu.css' type='text/css' media='screen'/>        
		<link rel='stylesheet' href='/public/css/pictures.css' type='text/css' media='screen'/>
    ...
    <script type='text/javascript' src='/public/js/jquery-ui-1.8.10.js'></script>
		<script type="text/javascript" src="/public/js/jquery-pictures.js"></script>
    ...
    if(isset($_SESSION["LAST_ACTIVITY"])){
      ...
      <div id="uploader" class="modalwindow tabContainers" title="'.$up_id.'">
        <form method="post" enctype="multipart/form-data" name="uploadForm" id="uploadForm">
          <div id="container" >
            <span class="normalText">Select an Image file to upload (limit 1MB)</span> <br>
            <span class="normalText">Allowed image types: .jpg, .png, .bmp, .gif, .tiff</span> <br>
            <input type="hidden" name="APC_UPLOAD_PROGRESS" id="progress_key" value="'.$up_id.'"/>
            <input class="normalText" style="color:#fff;" name="picFile" type="file" id="picFile"/>
            <br />
    ...
    <div id="pageHeader" class="shadowed">
      ...
      <div  id='siteMenu'>
		...
		<div id='pageBody'>
			<div id='picturesHeader'>
					<span class='pageTitle shadowed' style='position:absolute; left: 10px; top: 10px;'>Manage Your Profile Pictures</span>
					<ul id='pageMenu'>
					</ul>
			</div>
			<div id='picturesBody' class='shadowed'>
        <div id='loading' style='position:relative;margin:10px;height:32px;width:100%;'>
            <img id='loader' src='/public/images/loader.gif'/><label for='loader' class='fieldLabel' style='float:right;width:96%;padding:6px;'>Updating Information...</label>
        </div>
        <div id='output'></div>
				<div id='leftContainer'>
					<div id='controls' class='formopts' style='border-bottom:1px solid grey;padding-bottom:5px;'>
						<button id='upload' type='button'>+ New Picture</button>
					</div>
          <form id='pictureForm' name='pictureresume' enctype='multipart/form-data'>
            <div id='formBody'></div>
